...
- Redis:
  ...
  - DRAINPROCS :: number of processes to start that drain results from redis instance to memory. May be given with suffix 'N' to give number of nodes instead of processes. (Drainprocesses always take whole nodes, so this should be preferred.) Given to *invoke_sbatch.sh* after optional argument *--drainprocs*, set to 1N otherwise.
- R-Script invocation
...
1. invoke_sbatch.sh
   - Takes arguments:
     ...
     - DRAINPROCS (from --drainprocs argument)
...
2. sbatch.cmd
   ...
   - Does:
     - launches *runredis.sh* to create redis instance; waits for it to come up
     - launches *DRAINPROCS* *drainredis.R* processes on ceiling(*DRAINPROCS* / floor(*SLURM_MEM_PER_NODE* / 2048)) nodes
...
- [-] 200 runs for each learner x task on average, that's a lot.
  ...
  - [ ] plan now: use multiple redis instances, do manual sharding, only drain with one drain thread at a time with common queue
    - [ ] using BUCK single element that workers do locking wait for, otherwise keeping their private queues.
  - [ ] write sbatch output to its own directory
- [X] info to write out
